...
const db = new DatabaseSync("medicart.db");
...
if (count === 0) {
  ...
  for (const r of products) insert.run(...r);
}
...
app.post("/api/products", (req,res) => {
  ...
  res.status(201).json(db.prepare("SELECT * FROM products WHERE id=?").get(Number(result.lastInsertRowid)));
});
...
app.post("/api/orders", (req,res) => {
  ...
  try {
    ...
    for (const {p,qty} of validated) {
      addItem.run(orderId,p.id,qty,p.price);
